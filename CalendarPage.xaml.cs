...
namespace HelloSLC
{
    ...
    /// </summary>
    public sealed partial class CalendarPage : HelloSLC.Common.LayoutAwarePage
    {
           ...
           // this.DefaultViewModel["Items"] = events;
            ...
            if (navigationParameter != null)
            {
                ...
                if (students.Count > 0)
                {
                    ...
                    this.DefaultViewModel["Items"] = selectedStudent.eventObjs;

                    var selectedEvents = selectedStudent.eventObjs.First(c => c.id == selectedFeedItemId);
                    this.DefaultViewModel["selectedEvent"] = selectedEvents;
                }
            }
        ...
        private async void setupEvents()
        {
            ...
            {
                s.eventObjs = new List<Event>();
                if (s.id != null)
                {
                    List<Event> studentEvents = await getEventsByStudent(s.id);
                    foreach (Event eventObj in studentEvents)
                    {
                        s.eventObjs.Add(eventObj);
                        all.eventObjs.Add(eventObj);
                    }
                    students.Add(s);
                }
            }
             ...
             if (students.Count > 0 && selectedStudentId != null)
            {
                ...
                var selectedEvent = selectedStudent.eventObjs.First(c => c.id == selectedFeedItemId);
                this.DefaultViewModel["selectedEvent"] = selectedEvent;
            }
        }
